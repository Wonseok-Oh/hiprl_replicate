...
DIR_TO_VEC = [
    ...
    np.array((1, 0)),
    # Down (positive Y) #-- modified to be negative
    np.array((0, 1)),
    # Pointing left (negative X)
    np.array((-1, 0)),
    # Up (negative Y) #-- modified to be positive 
    np.array((0, -1)),
]
...
class WorldObj:
    ...
    def can_toggle(self):
        """Can this agent open/close this?"""
        return False
    ...
    @staticmethod
    def decode(type_idx, color_idx, state, objectId=None):
        ...
        # State, 0: close, 1: open, 2: locked
        is_open = state % 10 == 1
        is_locked = state == 2
        if state > 10:
            contain_id = state // 1000
            contain_color = state // 100
            contain_type = (state % 100) // 10
            contain = WorldObj.decode(contain_type, contain_color, 0, contain_id)
        else:
            contain = None
        ...
        if obj_type == 'wall':
            ...
            v = Ball(objectId, color)
        elif obj_type == 'key':
            v = Key(objectId, color)
        elif obj_type == 'box':
            v = Box(objectId, color, contain, is_open)
        elif obj_type == 'door':
...
class Key(WorldObj):
    def __init__(self, objectId, color='blue'):
...
class Box(WorldObj):
    def __init__(self, objectId, color, contains=None, isOpen=False):
    ...
    def can_pickup(self):
        if self.isOpen and self.contains != None:
            return self.contains.can_pickup()
        return False
    ...
    def encode(self):
        ...
        if self.isOpen and self.contains != None:
            ...
            state = 0
        return (OBJECT_TO_IDX[self.type], COLOR_TO_IDX[self.color], state, self.objectId)
    ...
    def decode(array):
        ...
        for i in range(width):
            for j in range(height):
                type_idx, color_idx, state, objectId = array[i, j]
                v = WorldObj.decode(type_idx, color_idx, state, objectId)
                grid.set(i, j, v)
...
#                print('placing object %s' %(obj.type))
    ...
    def step(self, action):
        ...
        # Pick up an object
            ...
            if fwd_cell and fwd_cell.can_pickup():
                if self.carrying is None and fwd_cell.type != 'box':
                    self.carrying = fwd_cell
                    self.carrying.cur_pos = np.array([-1, -1])
                    self.grid.set(*fwd_pos, None)
                elif self.carrying is None and fwd_cell.type == 'box':
    ...
    def render(self, mode='human', close=False, highlight=True, tile_size=TILE_PIXELS):
        ...
        for vis_j in range(0, self.agent_view_size):
            for vis_i in range(0, self.agent_view_size):
                ...
                abs_i, abs_j = self.get_world_coordinate(vis_i,vis_j)
    ...
    def get_world_coordinate(self, vx, vy):
        ax, ay = self.agent_pos
        dx, dy = self.dir_vec
        rx, ry = self.right_vec
