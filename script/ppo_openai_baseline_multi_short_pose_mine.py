...
#env = make_vec_env('MiniGrid-MyMultiHiPRLGridShortPose-v0', n_envs=8, start_index = 32)
env = gym.make('MiniGrid-MyMultiHiPRLGridShortPose-v0', process_num = 8)
#check_env(env)
#env = Monitor(env)
#env = DummyVecEnv([lambda: env])
#eval_callback = EvalCallback(env, best_model_save_path='./logs_multi_iehiprl_ent001_open_only/',
#                             log_path='./logs_multi_iehiprl_ent001_open_only/', eval_freq=200, n_eval_episodes=100,
#                             deterministic=True, render=False)
#model = PPO("MlpPolicy", env, verbose=1, policy_kwargs = policy_kwargs, n_steps=256, batch_size=64, 
#            ent_coef=0.01, n_epochs=10, learning_rate = 0.0002,
#            tensorboard_log="/home/morin/catkin_ws/src/hiprl_replicate/Visualization/Baselines/")
#model.learn(total_timesteps=300000, callback=eval_callback, tb_log_name="multi_iehiprl_ent001_open_only")
#model.learn(total_timesteps=50000, tb_log_name="test_stable_baseline3")

#model.save("ppo_multi_iehiprl_ent001_open_only")
...
model = PPO.load("multi_iehiprl_ent001_open_only30")
...
assert len(ep_reward) == 20
assert len(ep_success) == 20
print("*****************************")
print("Success rate: {}, Mean Makespan: {}, Mean episode reward sum: {}".format(mean_SR, mean_makespan, mean_rew))
print("*****************************")
'''
for i in range(30):
    model = PPO.load("multi_iehiprl_ent001_open_only" + str(i+1))
...
with open('Validation_Curve_IEHiPRL_multi', 'w') as f:
    write = csv.writer(f)
