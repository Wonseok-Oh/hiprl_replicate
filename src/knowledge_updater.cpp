...
void KnowledgeUpdater::resetCallback(const rosplan_knowledge_msgs::processReset::ConstPtr &msg){
	m_search_mode = true;
	m_keep_mode = false;
	m_bring_mode = false;
	m_prev_pos_x = 0;
...
bool KnowledgeUpdater::process_action_effect(hiprl_replicate::ActionExecution::Request &req,
...
bool KnowledgeUpdater::check_precondition(hiprl_replicate::ActionExecution::Request &req,
										hiprl_replicate::ActionExecution::Response &res){
...
void KnowledgeUpdater::goalCallback(const geometry_msgs::PoseStampedConstPtr &msg){
...
void KnowledgeUpdater::poseCallback(const geometry_msgs::PoseStampedConstPtr &msg){
	rosplan_knowledge_msgs::KnowledgeUpdateServiceArray srv;
	rosplan_knowledge_msgs::KnowledgeItem item;
	int agent_pos_x = (msg->pose.position.x - m_resolution / 2) / m_resolution;
	int agent_pos_y = m_grid_size - (msg->pose.position.y + m_resolution / 2.0) / m_resolution;
	cout << "x,y: " << agent_pos_x << ", " << agent_pos_y << endl;
	...
	value.value.assign(string("robot"));
	item.values.push_back(value);

	value_loc.key.assign(string("l"));
	string location = string("f") + to_string(agent_pos_y) + string("-") + to_string(agent_pos_x) + string("f");
	...
	if (agent_pos_x != m_prev_pos_x || agent_pos_y != m_prev_pos_y){
		...
		value_prev.value.assign(string("robot"));
	...
	while (m_client.call(srv) != true){
		ROS_ERROR("KnowledgeUpdater::poseCallback: Failed to call service knowledge_update");
		sleep(1);
	}
	ROS_INFO("poseCallback Knowledge Update Service Result: %d", srv.response.success);
	m_prev_pos_x = agent_pos_x;
	m_prev_pos_y = agent_pos_y;

	return;
}

void KnowledgeUpdater::observationCallback(const hiprl_replicate::Obs::ConstPtr &msg){
	...
	for (index = 0; index < msg->type_id_list.size(); index++){
		int object_pos_x = msg->object_pos_list.at(2*index);
		int object_pos_y = msg->object_pos_list.at(2*index+1);

		item = rosplan_knowledge_msgs::KnowledgeItem();
		if (msg->type_id_list.at(index) == msg->EMPTY){
			// set clear cell true
			srv.request.update_type.push_back(static_cast<int>(Update_type::add_knowledge));
			...
			item.values.push_back(value);
			srv.request.knowledge.push_back(item);
		} else if (msg->type_id_list.at(index) == msg->WALL){
			...
			item.values.push_back(value);
			srv.request.knowledge.push_back(item);
		} else if (msg->type_id_list.at(index) == msg->GOAL){
		...
		} else if (msg->type_id_list.at(index) == msg->BOX){
			if (find(m_seen_box_id_list.begin(), m_seen_box_id_list.end(), msg->object_id_list.at(index)) != m_seen_box_id_list.end()){
				// already seen box --> manage only opened or not at below

			} else {
				m_seen_box_id_list.push_back(msg->object_id_list.at(index));
				...
				value.value.assign(string("box") + to_string(msg->object_id_list.at(index)));
				item.values.push_back(value);
				...
				value_openable.value.assign(string("box") + to_string(msg->object_id_list.at(index)));
				...
				item.instance_name.assign(string("box") + to_string(msg->object_id_list.at(index)));
				srv.request.knowledge.push_back(item);
/*
				...
				if (m_search_mode){
					...
					value_checked.value.assign(string("box") + to_string(msg->object_id_list.at(index)));
					item.values.push_back(value_checked);
					srv.request.knowledge.push_back(item);
				}
			}

			if (msg->object_state_list.at(index) % 2 == 1){ // opened box
				// set opened true
				srv.request.update_type.push_back(static_cast<int>(Update_type::add_knowledge));
				...
				string receptacle = string("box") + to_string(msg->object_id_list.at(index));
				...
				if (find(m_checked_box_id_list.begin(), m_checked_box_id_list.end(), msg->object_id_list.at(index)) != m_checked_box_id_list.end()){
					...
					m_checked_box_id_list.push_back(msg->object_id_list.at(index));
					// set checked true
					srv.request.update_type.push_back(static_cast<int>(Update_type::add_knowledge));
					diagnostic_msgs::KeyValue value_checked;
					item = rosplan_knowledge_msgs::KnowledgeItem();
				...
				if (msg->object_state_list.at(index) > 10){
					int object_type_id = msg->object_state_list.at(index) % 100;
					int contain_id = msg->object_state_list.at(index) / 1000;
		...
		} else if (msg->type_id_list.at(index) == msg->BALL){
			// add instance if unseen object
			if (find(m_seen_obj_id_list.begin(), m_seen_obj_id_list.end(), msg->object_id_list.at(index)) != m_seen_obj_id_list.end()){
				// already checked box --> do not add instance
			} else {
				ROS_INFO("ball out of box detected, id: %d", msg->object_id_list.at(index));
				m_seen_obj_id_list.push_back(msg->object_id_list.at(index));
				// add instance
				srv.request.update_type.push_back(static_cast<int>(Update_type::add_knowledge));
				diagnostic_msgs::KeyValue value_add_inst;
				item = rosplan_knowledge_msgs::KnowledgeItem();
				...
				item.instance_name.assign(string("ball") + to_string(msg->object_id_list.at(index)));
				srv.request.knowledge.push_back(item);
			}
			...
			item.attribute_name.assign(string("object_at_location"));
			value_obj.key.assign(string("o"));
			string obj = string("ball") + to_string(msg->object_id_list.at(index));
